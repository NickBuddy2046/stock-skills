...
```
{Symbol} ({code}) news digest — Source: stock data provider
...
When you do, prepend: _"Below is from a web search — not stock data."_ / _"以下为网络搜索结果,非长桥数据。"_ / _"以下為網絡搜尋結果,非長橋數據。"_
...
| `news` returns empty            | "{symbol} has no recent news in stock data — switching to WebSearch (note: web data, not Longbridge)." |
