...
Prompt-only analysis skill. Retrieves SEC filings via the stock data provider and extracts investment-relevant signals: financial KPIs (10-K/10-Q), material events (8-K), governance/compensation (Proxy), and insider behaviour (Form 4).
...
6. **Output** structured analysis (template below). Cite stock data provider.
...
```
{Symbol} {Filing type} analysis — Source: stock data provider
