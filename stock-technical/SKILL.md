...
- **If `longbridge` CLI is installed** (via `stock-market-data` or standalone): run the command above directly.
- **If CLI is unavailable**: fall back to the stock MCP server — call the kline/OHLCV tool at runtime to fetch the same data.
...
| User wants | Use |
|---|---|
| Raw K-line / quote data | `stock-market-data` |
| Options Greeks / IV | `stock-derivatives` |
| Quantitative strategies | `stock-quant` |
