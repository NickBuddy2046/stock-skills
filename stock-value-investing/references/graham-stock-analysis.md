...
3. **Fetch raw data via stock CLI first** (parallel where possible). See [§CLI](#cli).
...
Each WebSearch-sourced figure must be tagged `[Source: WebSearch — <publisher>, <date>]` in the appendix; do **not** mix it silently with stock data.
