---
name: stock-value-investing
description: |
  Value investing analysis using Graham (NCAV/net-net/defensive-investor) and Buffett (economic moat/ROE/FCF) methodologies. Covers single-stock diagnostics and batch screening for both Graham cigar-butt and Buffett quality-compounder criteria. Runs cross-statement reconciliation before scoring. Data from stock CLI first, MCP fallback, WebSearch only for genuine gaps.
...
Graham and Buffett value investing analysis via the stock data provider.
...
| User wants | Use |
|---|---|
| General value screen (low PE/PB) | `stock-fundamentals` (value-screen) |
| DCF intrinsic value | `stock-fundamentals` (dcf) |
| Analyst ratings / institutional view | `stock-research` |
| Post-earnings analysis | `stock-earnings` |
