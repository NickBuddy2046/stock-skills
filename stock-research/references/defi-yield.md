...
> **Data scope notice**: Longbridge provides real-time spot prices for major cryptocurrencies (`.HAS` suffix, e.g. `BTCUSD.HAS`, `ETHUSD.HAS`). DeFi protocol metrics (APY, TVL, utilisation rates) are **not available** via the stock CLI — they must be sourced via WebSearch (DefiLlama, CoinGecko, protocol dashboards).
...
DeFi protocol data is **not available** via the stock CLI. Use WebSearch to retrieve live APY/TVL from DefiLlama, CoinGecko, or protocol dashboards.
