...
Prompt-only 港股打新分析。给定一只新股代码或公司名，输出「推荐打新 / 中性观望 / 建议回避」三档评级 + 四维评分明细 + 招股书精华 + 风险提示 + 申购手数参考；或在日历模式下输出未来 4 周打新日程表 + 评级速览。报告结尾固定输出「数据来源明细」附录，每一行注明来源（stock CLI 子命令 / MCP 工具 / WebSearch URL）、抓取时间和数据期间。
...
WebSearch 是兜底，**不是默认路径**。仅在 stock CLI + MCP 都拿不到时使用，且必须在附录显式标 `[WebSearch — <publisher>, <date>, <url>]`，禁止伪造来源。
