...
Prompt-only analysis skill. Orchestrates stock CLI commands to surface analyst consensus estimates, estimate revision trends, beat/miss history, and post-earnings announcement drift (PEAD) signals.
...
4. Output structured report; cite **stock data provider**; end with disclaimer.

## Output template

```
{Symbol} ({code}) Analyst Consensus — Source: stock data provider
As of: {date}
