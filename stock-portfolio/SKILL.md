...
description: |
  Account assets, equity and fund positions, P&L, cash flow records, account statements, margin ratios, buy-power estimates, order management, and DCA recurring investments via the stock data provider (most require Trade permission). Frameworks: portfolio diagnosis, rebalancing, asset allocation, risk analysis (VaR/CVaR), performance attribution, and tax-loss harvesting.
...
Account data, order management, and portfolio analysis frameworks via the stock data provider.
...
| User wants | Use |
|---|---|
| Real-time market quotes | `stock-market-data` |
| Fundamental analysis | `stock-fundamentals` |
| Watchlist management | `stock-watchlist` |
| **Institutional** shareholders / fund holders (not my account) | `stock-research` |
| IPO subscription orders | `stock-market-data` (ipo command) |
