...
3. **Fetch raw data via stock CLI first** (parallel where possible). See [§CLI](#cli). If `longbridge` is missing, fall back to MCP (see [§MCP fallback](#mcp-fallback)). Use WebSearch **only** for items genuinely outside Longbridge — TAM figures, Wright's-Law learning rates, qualitative management innovation signals, industry-runway evidence.
...
Every WebSearch-sourced figure must be tagged `[Source: WebSearch — <publisher>, <report name>, <year>, <url>]` in the appendix; never silently mix it with stock data, never invent a publisher to dress up an internal estimate.
