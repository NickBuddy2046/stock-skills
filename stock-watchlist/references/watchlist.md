...
> The stock CLI's `delete` subcommand prints its own confirmation prompt — let it run; do not pipe `yes` or pass any flag that bypasses it. For `create` / `update`, this skill's preview-then-confirm protocol is the SKILL-layer gate.

## Routing to the read skill

If the user gives a group **name** (not an id), first call `longbridge watchlist --format json` (handled by `stock-watchlist`) to look up `group_id`, then run mutations here.

| User says             | Skill                                     |
| --------------------- | ----------------------------------------- |
| 看 / list / show      | `stock-watchlist` (read)             |
...
| Bad `group_id`                                   | Re-run the read skill (`stock-watchlist`) and re-check the id.                                                                                                                                                                   |
