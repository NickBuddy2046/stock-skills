...
Prompt-only analysis skill. Orchestrates stock CLI commands to answer _"is X expensive?"_ across three dimensions: current snapshot, historical percentile, industry context.
...
5. **Output the three sections** (template below). Cite **stock data provider**.

## Output template

```
{Symbol} ({code}) valuation snapshot — Source: stock data provider
...
- **Must** cite "stock data provider" / "数据来源:长桥证券" / "數據來源:長橋證券".
